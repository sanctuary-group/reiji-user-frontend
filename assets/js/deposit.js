...
(function () {
  ...
  function saveEntry() {
    ...
    if (!date) { showGlobalToast('日付を入力してください', 'error'); return; }
    if (amount <= 0) { showGlobalToast('金額を入力してください', 'error'); return; }
    if (!account) { showGlobalToast('口座を入力してください', 'error'); return; }
    if (!method) { showGlobalToast('方法を入力してください', 'error'); return; }
